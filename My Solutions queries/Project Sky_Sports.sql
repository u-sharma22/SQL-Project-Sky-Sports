...
select team, 100*(wins/matches_played)  as wins_percent from group_stage_team_stats;
...
select team, round(100*(draws/matches_played),2) as draws_percent from group_stage_team_stats;
...
#10WASQ to show percent of losses with respect to matches_played by each team in ascending order by losses and name the resulting column as losses_percent
select team, round(100*(losses/matches_played),2) as losses_percent from group_stage_team_stats order by losses_percent;
...
select * from group_stage_team_stats gsts  where expected_goal_scored < exp_goal_conceded;
...
select * from group_stage_team_stats order by exp_goal_difference_per_90;

#16.	Write an SQL query to show team which has maximum number of players_used
select team from overall_wc_stats ows where players_used = (select max(players_used) from overall_wc_stats ows2);

#17.	Write an SQL query to show each team name and avg_age in ascending order by avg_age
select team, avg_age from overall_wc_stats ows order by avg_age;
...
select team, max(fouls) from overall_wc_stats;
#or
...
select team,  `group`   from group_stage_team_stats having `group` = 6;


#34. Write a SQL query to show Australia belongs to which group
select team, `group` from group_stage_team_stats gsts where team = 'Australia';
...
select gs.`group`, max(gs.expected_goal_scored) as max_exp_goal_scored from group_stage_team_stats gs group by gs.`group` order by max_exp_goal_scored;

#37. Write a SQL query to show group, minimum exp_goal_conceded by each group in descending order by exp_goal_conceded
select gs.`group`, min(gs.exp_goal_conceded) as min_exp_goal_conceded from group_stage_team_stats gs 
group by gs.`group` order by min_exp_goal_conceded desc;
...
select gs.`group`, avg(gs.exp_goal_difference_per_90) as avg_exp_goal_diff_per_90 from group_stage_team_stats gs
group by gs.group order by avg_exp_goal_diff_per_90;
